...
Given an array integer arr[] , representing the values of coins arranged in a row. Two players play a game by picking coins alternately. At each turn, a player can pick a coin from either the beginning or the end of the array. Both players follow a greedy strategy, i.e., they always pick the coin with the maximum value among the two available ends.
...
class Solution
{
	...
	int coin(vector<int>& arr)
	{
		...
		while (left != right)
		{
		  //  Take the maximum of two ends
			if (arr[left] > arr[right])
				left++;
			else
				right--;
		}
        // Since left == right at end, so taking arr[left] is same as for right
		return arr[left];
	}
};
